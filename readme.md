...
The device mentioned in this page, Tilgin HG2331, has an EULA stating that reverse engineering is strictly prohibited. I have complied with the EULA and never touched the software that is installed with the device.
...
Well, you might want to get a MicroSD card for this.
...
This caused the HGW to get an IP address from my internal network and think that my RouterBoard is an TR069 server. Luckily RouterOS responded with `Not Implemented`.
...
* If the L7 rule(s) gets requests that are DNS requests, redirect those to the DNS of the VoIP network using `dst-nat`. This might change on different regions in the country.
...
Basically, use SSL and don't store any private keys on the HGW.
...
I didn't want to release this. However, the more people having all kinds of issues with their HGW, I **needed** to release this. I am a student, I must be studying. But the guilt in me always pushes me to help everyone in any field that I have experience of.
